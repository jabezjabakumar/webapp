pipeline {
  ...
  stages {
    stage ('Initialize') {
      ...
      steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no root@13.233.168.57 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://13.127.63.73:8080/webapp/ -g gen.conf -r zaphtmlreport.html" || true'
        }
      }
    }
